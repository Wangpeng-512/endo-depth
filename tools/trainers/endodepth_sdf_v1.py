...
from networks.endodepth_v2 import ResnetAttentionEncoder, DepthUnet
...
class EndoDepthTrainer(Trainer):
    ...
    def make_models(options, *args, **kwargs):
        ...
        depth = DepthUnet(options.num_layers, len(options.frame_ids), False)
        param += list(depth.parameters())

        models = OrderedDict([
            # ("encoder", depth.backbone),
            ("depth", depth)
        ])
        return models, param
